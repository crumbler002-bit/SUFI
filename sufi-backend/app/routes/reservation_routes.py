...
def get_restaurant_availability(
    ...
    """Return a bookable slot list for a single restaurant."""
    ...
    while current_minutes < closing_hour * 60:
        ...
        if slot_time >= now:
            available_tables = search_available_tables(db, restaurant_id, guests, slot_time)
            slots.append(
                {
